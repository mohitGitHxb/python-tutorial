...
def plot_model_performance(df, metrics, stocks):
    ...
    for metric in metrics:
        ...
        ax = sns.barplot(x='Model', y=metric, hue='Stock', data=df[df['Stock'].isin(stocks)], palette="muted")
        
        # Set a clear title and labels
        plt.title(f'Comparison of {metric} Across Models and Stocks With Transactional Cost Of 0.3% Per Trade', fontsize=14, pad=10)
...
if __name__ == "__main__":
    ...
    models = ['Attention_CNN_BiLSTM_STFT', 'MFA_STFT', 'Attention_CNN_BiLSTM_Multi_Indicator', 'MFA_Multi_indicator']
